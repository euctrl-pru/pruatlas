...
ecac_nw <- firs_nm_406 %>%
  dplyr::filter(icao %in% ecac_nw_codes, min_fl <= 0, 0 <= max_fl) %>%
...
```{r, ecac-oc}
ecac_oc_codes <- c("BI", "EG", "EK", "EN")
ecac_oc <- pruatlas::firs_nm_406 %>% 
...
ecac_sw_codes <- c("GC",
             ...
             NULL)
ecac_sw <- pruatlas::firs_nm_406 %>% 
  dplyr::filter(icao %in% ecac_sw_codes, min_fl <= 0, 0 <= max_fl) %>%
...
ecac_ne_codes <- c("EE", "EF", "EN", "EP", "ES", "EV", "EY",
             "LK", "LZ", "UK")
ecac_ne <- pruatlas::firs_nm_406 %>% 
  dplyr::filter(icao %in% ecac_ne_codes, min_fl <= 0, 0 <= max_fl) %>%
  ...
  mutate(min_fl = 0, max_fl = 999)
plot_country_fir(icao_id = "ECAC_NE", name = "ECAC North East", firs = ecac_ne)
```
