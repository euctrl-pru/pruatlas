...
```{r, ecac-oc}
ecac_oceanic() %>% 
  plot_country_fir(icao_id = "ECAC_OC",
                   name = "ECAC Oceanic",
                   firs = .)
```
...
```{r ecac-nw}
ecac_northwest() %>% 
  plot_country_fir(icao_id = "ECAC_NW",
...
```{r ecac-sw}
ecac_southwest() %>% 
  plot_country_fir(icao_id = "ECAC_SW",
...
```{r, ecac-se}
ecac_southeast() %>% 
  plot_country_fir(icao_id = "ECAC_SE",
...
```{r, ecac-whole, eval=FALSE, echo=FALSE}
firs <- read_sf(here::here("inst", "extdata","icao_firs.geojson")) %>% 
